...
def measure_interfaces(label, label_matrix, transition, void, time):
    ...
    for t in range(1,len(time)):
        ...
        if (pore_filling == t).any():
            ...
            w_w_inter = interface_extraction(wet, n_wet)
...
#    step_size[1:] = np.diff(time)
    ...
    # FIXME: sample_data['label'].data are already just the relevant labels --> mismatch between filling dynamics and surface measurment
