...
for file in file_list:
    if file[:3] == 'dyn':
        ...
        sample_profiles = cp.zeros((2016,271))
        any_water = transitions>0
        
        for ts in range(1,271):
            if ts in time_steps:
                water = cp.bitwise_and(any_water,(transitions<ts+1))
                profile = cp.count_nonzero(water, axis = (0,1))
                sample_profiles[:,ts] = profile
            else:
                sample_profiles[:,ts] = sample_profiles[:,ts-1]
        
        profiles[:,:,cc] = cp.asnumpy(sample_profiles)
        cc = cc + 1
